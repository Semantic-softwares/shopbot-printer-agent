...
import { formatUsbHex } from '../../../../shared/utils/printer-display.util';

@Component({
  selector: 'app-add-printers',
  standalone: true,
  imports: [CommonModule, FormsModule, LoadingSpinnerComponent, CopyFieldComponent],
...
export class AddPrintersComponent {
  ...
  usbRegistrationSummary(printer: any): string {
    return [
      ...
      `Vendor ID: ${printer.vendorId} (${this.formatHex(printer.vendorId)})`,
      `Product ID: ${printer.productId} (${this.formatHex(printer.productId)})`,
      `Device Name: ${printer.vendorId}:${printer.productId}`,
    ].join('\n');
  }
